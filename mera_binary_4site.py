...
S2.shape = (2,2,2,2,2,2,2,2)
Sz.shape = (2,2,2,2,2,2,2,2)
...
for it in range(10000):
    m = np.einsum("jkmn,imo,nlp,opq->ijklq", U, w1, w2, w3)
    ...
    S2_curr = np.einsum("q,ijklq,ijklmnop,mnopr,r->",v,m,S2,m,v)
    Sz_curr = np.einsum("q,ijklq,ijklmnop,mnopr,r->",v,m,Sz,m,v)
    #E_curr = np.einsum("ijkl,jkmn,imo,nlp,opq,q->",sigma,U,w1,w2,w3,v)
    
    #mera_state = np.einsum('i,jki,lmj,nok,pqmn->lpqo',v,w3,w1,w2,U)
    #normsq = np.einsum('ijkl,ijkl->',mera_state,mera_state)
    #print " Norm: %12.8f " % normsq
    ...
    for it2 in range(1):
        ...
        #tmp1 = np.einsum('i,jki,lmj,nok,pqmn,lpqorstu,stvw,rvx,wuy,xyz,z->',v,w3,w1,w2,U,H,U,w1,w2,w3,v)
        ...
        #tmp2 = np.einsum('i,jki,lmj,nok,pqmn,lpqorstu,stvw,rvx,wuy,xyz,z->',v,w3,w1,w2,U,H,U,w1,w2,w3,v)
        #print "E: %12.8f %12.8f" %(tmp1,tmp2)
